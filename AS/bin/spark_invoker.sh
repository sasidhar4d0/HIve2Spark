...
"xcu") conf_file="/hdfs/app/HadoopCPS/ETL/conf/cps-spark-clustermode-xlarge.conf"
	;;
*) echo $confType 
echo "config_type not found or not declared, running in clustermode-default.
The options are:
...
function invoke_spark
{
 $SPARK_CORE/spark-submit 	--class SparkHqlWrapper \
				...
				$SPARK_JAR/sparkhqlwrapper_2.11-0.8.jar  $file /hdfs/app/HadoopCPS/ETL/conf/parameters.txt
}   # end of invoke_spark
